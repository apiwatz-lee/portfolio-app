...
const Navbar = ({page}) => {
    ...
    const mobileMenu = links.map((link,index)=>{
        return ( 
            <li key={index} className={`flex justify-center rounded-md w-full duration-300 hover:bg-[#3E3F42] hover:text-white ${link.name === 'CONTACT'?`border border-slate-400`:null}`}>
